 ...
 * 
 * The base of a {@link FMEPPTFreeModel} is a {@link VirtualModel} which aims to federate the {@link FMEConceptualModel} with a diagram
...
@ModelEntity
@XMLElement
@ImplementationClass(FMEPPTFreeModel.FMEDiagramFreeModelImpl.class)
public interface FMEPPTFreeModel extends FMEFreeModel {
	...
	public abstract class FMEDiagramFreeModelImpl extends FMEFreeModelImpl implements FMEPPTFreeModel {

		private static final Logger logger = FlexoLogger.getLogger(FMEPPTFreeModel.class.getPackage().getName());
		...
		public DiagramPalette getConceptsPalette() throws FlexoException {
			if (getDiagramSpecification().getPalette(PALETTE_NAME) == null) {
				...
				createPalette.setNewPaletteName(FMEPPTFreeModel.PALETTE_NAME);
		...
		public DiagramPaletteElement createPaletteElementForConcept(FlexoConcept grConcept, FlexoConcept concept,
			...
			if (paletteElementGR.getShapeSpecification().getShapeType() == ShapeType.SQUARE
				...
				paletteElementGR.setX(px * FMEPPTFreeModel.PALETTE_GRID_WIDTH + 15);
				paletteElementGR.setY(py * FMEPPTFreeModel.PALETTE_GRID_HEIGHT + 10);
				paletteElementGR.setWidth(30);
				paletteElementGR.setHeight(30);
			}
			else {
				paletteElementGR.setX(px * FMEPPTFreeModel.PALETTE_GRID_WIDTH + 10);
				paletteElementGR.setY(py * FMEPPTFreeModel.PALETTE_GRID_HEIGHT + 10);
		...
		protected void configureGRFlexoConcept(FlexoConcept returned, FlexoConcept concept, FlexoConcept containerConceptGR,
			...
			if (containerConceptGR == null) {
				...
				addShape.setContainer(new DataBinding<>(DropScheme.TARGET_KEY + "." + FMEPPTFreeModel.SHAPE_ROLE_NAME));
			}
		...
		protected void configureGRRelationalFlexoConcept(FlexoConcept returned, FlexoConcept concept, FlexoConcept fromConceptGR,
			...
			addConnector.setFromShape(new DataBinding<>(LinkScheme.FROM_TARGET_KEY + "." + FMEPPTFreeModel.SHAPE_ROLE_NAME));
			addConnector.setToShape(new DataBinding<>(LinkScheme.TO_TARGET_KEY + "." + FMEPPTFreeModel.SHAPE_ROLE_NAME));
